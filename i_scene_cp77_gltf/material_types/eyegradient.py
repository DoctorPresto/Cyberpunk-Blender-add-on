    ...
    def create(self, Data, Mat):
        ...
        if "Albedo" in Data:
            aImgNode = CreateShaderNodeTexImage(
                CurMat, self.BasePath + Data["Albedo"], -1000, 250, 'Albedo', self.image_format
                )
        ...
        if "Normal" in Data:
            nMap = CreateShaderNodeNormalMap(
                CurMat, self.BasePath + Data["Normal"], -600, -300, 'Normal', self.image_format
                )
        ...
        if "IrisMask" in Data:
            iMask = CreateShaderNodeTexImage(
                CurMat, self.BasePath + Data["IrisMask"], -1000, 500, 'Iris Mask', self.image_format
                )
        ...
        if "IrisColorGradient" in Data:
            ...
            for Entry in profile["gradientEntries"]:
                if counter == 0:
                    ...
                    colr = Entry["color"]
                    igradNode.color_ramp.elements[0].color = (float(colr["Red"]) / 255, float(colr["Green"]) / 255,
