    ...
    def create(self, Data, Mat):
        ...
        if "DiffuseTexture" in Data:
            if not isGradientMapRecolor:
                dImg = imageFromRelPath(
                        Data["DiffuseTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath,
                        image_format=self.img_format
                        )
            else:
                dImg = imageFromRelPath(
                        Data["DiffuseTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath,
                        image_format=self.img_format, isNormal=True
                        )
        ...
        if "MaskTexture" in Data:
            aImg = imageFromRelPath(
                    Data["MaskTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath, image_format=self.img_format,
                    isNormal=True
                    )
        ...
        if "NormalAlphaTex" in Data:
            nAImg = imageFromRelPath(
                    Data["NormalAlphaTex"], DepotPath=self.BasePath, ProjPath=self.ProjPath,
                    image_format=self.img_format
                    )
        ...
        if "RoughnessTexture" in Data:
            rImg = imageFromRelPath(
                    Data["RoughnessTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath,
                    image_format=self.img_format, isNormal=True
                    )
        ...
        if "MetalnessTexture" in Data:
            mImg = imageFromRelPath(
                    Data["MetalnessTexture"], DepotPath=self.BasePath, ProjPath=self.ProjPath,
                    image_format=self.img_format, isNormal=True
                    )
