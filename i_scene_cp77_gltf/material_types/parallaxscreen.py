...
from ..materials.blender.nodes import CreateShaderNodeRGB, CreateShaderNodeValue, bsdf_socket_names, createHash12Group, createLerpGroup, createVecLerpGroup, create_node, loc

from .mat_common import MaterialTypeBase, add_group_node, create_param_value_nodes, get_or_build_node_group, set_scene_fps_driver

from .scalar_params import component_value, scalar_parameter_data, scalar_value
from .parallax_common import (
    create_camera_forward_vector,
    create_mip_sampled_texture,
    create_scroll_group,
    create_scroll_uv_group,
)

# parallaxscreen.fx:73-74 forces the ghost layers to these mip levels.
_LAYER_MIP_LEVELS = {2: 2, 3: 3}
_LAYER_MIP_TAPS = 9

_create_scroll_group = partial(create_scroll_group, transparent=False)
_create_scroll_uv_group = partial(create_scroll_uv_group, transparent=False)

_VALUE_PARAM_SPECS = (
    ("BlinkingSpeed", "blinkingSpeed", 450, "BlinkingSpeed", 0.0),
    ("LayersSeparation", "layersSeparation", 100, "LayersSeparation", 0.0),
    ("ScanlinesIntensity", "scanlinesIntensity", 0, "ScanlinesIntensity", 1.0),
    ("ImageScale", "imageScale", -200, "ImageScale", 1.0),
    ("ScrollSpeed1", "scrollSpeed1", -250, "ScrollSpeed1", 0.0),
    ("ScrollStepFactor1", "scrollStepFactor1", -300, "ScrollStepFactor1", 0.0),
    ("ScrollMaskHeight1", "scrollMaskHeight1", -350, "ScrollMaskHeight1", 0.0),
    ("ScrollMaskStartPoint1", "scrollMaskStartPoint1", -400, "ScrollMaskStartPoint1", 0.0),
    ("ScrollSpeed2", "scrollSpeed2", -500, "ScrollSpeed2", 0.0),
    ("ScrollStepFactor2", "scrollStepFactor2", -550, "ScrollStepFactor2", 0.0),
    ("ScrollMaskHeight2", "scrollMaskHeight2", -600, "ScrollMaskHeight2", 0.0),
    ("ScrollMaskStartPoint2", "scrollMaskStartPoint2", -650, "ScrollMaskStartPoint2", 0.0),
    ("IsBroken", "isBroken", -700, "IsBroken", 0.0),
    ("ScrollVerticalOrHorizontal", "scrollVerticalOrHorizontal", -750, "ScrollVerticalOrHorizontal", 0.0),
    ("Emissive", "emissive", -800, "Emissive", 1.0),
    ("EmissiveEV", "emissive", -800, "Emissive", 1.0),
    ("FixForBlack", "fixForBlack", -1000, "FixForBlack", 0.0),
    ("Metalness", "metalness", -1050, "Metalness", 0.0),
    ("Roughness", "roughness", -1100, "Roughness", 0.5),
    ("RoughnessScale", "roughness", -1100, "Roughness", 0.5),
    )

_IMAGE_PARAM_SPECS = (
    ("ParalaxTexture", "parImg"),
    ("BlinkingMaskTexture", "blinkImg"),
    ("ScanlineTexture", "scanLineImg"),
    ("ScrollMaskTexture", "scrollMaskImg"),
    )
...
def _create_l1_group():
    inputs = (('NodeSocketVector', 'newUV'),)
    outputs = (('NodeSocketVector', 'l1'),)

    def build(group):
        group_in = create_node(group.nodes, "NodeGroupInput", (-800, 0))
        group_out = create_node(group.nodes, "NodeGroupOutput", (200, 0))
        separate = create_node(group.nodes, "ShaderNodeSeparateXYZ", (-650, 0))
        clamp_x = create_node(group.nodes, "ShaderNodeClamp", (-500, 25))
        clamp_y = create_node(group.nodes, "ShaderNodeClamp", (-500, -25))
        combine = create_node(group.nodes, "ShaderNodeCombineXYZ", (-350, 0))
        group.links.new(group_in.outputs[0], separate.inputs[0])
        group.links.new(separate.outputs[0], clamp_x.inputs[0])
        group.links.new(separate.outputs[1], clamp_y.inputs[0])
        group.links.new(clamp_x.outputs[0], combine.inputs[0])
        group.links.new(clamp_y.outputs[0], combine.inputs[1])
        group.links.new(combine.outputs[0], group_out.inputs[0])

    return get_or_build_node_group("l1", inputs, outputs, build)


def _create_layer_uv_group(layer):
    group_name = f"l{layer}"
    scale_socket = 'l2' if layer == 2 else 'LayersSeparation'
    output_name = 'newRandomOffset' if layer == 2 else 'l3'
    inputs = (
        ('NodeSocketVector', 'modUV'),
        ('NodeSocketVector', 'newUV'),
        ('NodeSocketFloat', scale_socket),
        )
    outputs = (('NodeSocketVector', output_name),)

    def build(group):
        group_in = create_node(group.nodes, "NodeGroupInput", (-800, 0))
        group_out = create_node(group.nodes, "NodeGroupOutput", (300 if layer == 3 else 200, 0))
        vec_mul = create_node(group.nodes, "ShaderNodeVectorMath", (-600, 0), operation="MULTIPLY")
        offset = vec_mul
        if layer == 3:
            vec_mul2 = create_node(group.nodes, "ShaderNodeVectorMath", (-450, 0), operation="MULTIPLY")
            vec_mul2.inputs[1].default_value = (2, 2, 2)
            offset = vec_mul2
        vec_add = create_node(group.nodes, "ShaderNodeVectorMath", (-300 if layer == 3 else -450, 0), operation="ADD")
        separate = create_node(group.nodes, "ShaderNodeSeparateXYZ", (-150 if layer == 3 else -300, 0))
        clamp_x = create_node(group.nodes, "ShaderNodeClamp", (0 if layer == 3 else -150, 25))
        clamp_y = create_node(group.nodes, "ShaderNodeClamp", (0 if layer == 3 else -150, -25))
        combine = create_node(group.nodes, "ShaderNodeCombineXYZ", (150 if layer == 3 else 0, 0))
        group.links.new(group_in.outputs[0], vec_mul.inputs[0])
        group.links.new(group_in.outputs[2], vec_mul.inputs[1])
        if layer == 3:
            group.links.new(vec_mul.outputs[0], offset.inputs[0])
        group.links.new(group_in.outputs[1], vec_add.inputs[0])
        group.links.new(offset.outputs[0], vec_add.inputs[1])
        group.links.new(vec_add.outputs[0], separate.inputs[0])
        group.links.new(separate.outputs[0], clamp_x.inputs[0])
        group.links.new(separate.outputs[1], clamp_y.inputs[0])
        group.links.new(clamp_x.outputs[0], combine.inputs[0])
        group.links.new(clamp_y.outputs[0], combine.inputs[1])
...
class ParallaxScreen(MaterialTypeBase):
    def createStepGroup(self):
        stepGroup = bpy.data.node_groups.get('step')
        if stepGroup is not None:
            return stepGroup
        else:
            stepGroup = new_tracked_datablock("node_groups", "step", "ShaderNodeTree")
    ...
    def colorlessTexGroup(self):
        colorlessTexGroup = bpy.data.node_groups.get('colorlessTex')
        if colorlessTexGroup is not None:
            return colorlessTexGroup
        else:
            colorlessTexGroup = new_tracked_datablock("node_groups", "colorlessTex", "ShaderNodeTree")
    ...
    def create(self, Data, Mat):
        ...
        shader_nodes = create_param_value_nodes(
            CurMat, scalar_parameter_data(Data, _VALUE_PARAM_SPECS), _VALUE_PARAM_SPECS
            )

        blinkingSpeed = shader_nodes.get("blinkingSpeed")
        layersSeparation = shader_nodes.get("layersSeparation")
        scanlinesIntensity = shader_nodes.get("scanlinesIntensity")
        imageScale = shader_nodes.get("imageScale")
        scrollSpeed1 = shader_nodes.get("scrollSpeed1")
        scrollStepFactor1 = shader_nodes.get("scrollStepFactor1")
        scrollMaskHeight1 = shader_nodes.get("scrollMaskHeight1")
        scrollMaskStartPoint1 = shader_nodes.get("scrollMaskStartPoint1")
        scrollSpeed2 = shader_nodes.get("scrollSpeed2")
        scrollStepFactor2 = shader_nodes.get("scrollStepFactor2")
        scrollMaskHeight2 = shader_nodes.get("scrollMaskHeight2")
        scrollMaskStartPoint2 = shader_nodes.get("scrollMaskStartPoint2")
        isBroken = shader_nodes.get("isBroken")
        scrollVerticalOrHorizontal = shader_nodes.get("scrollVerticalOrHorizontal")
        emissive = shader_nodes.get("emissive")
        fixForBlack = shader_nodes.get("fixForBlack")
        metalness = shader_nodes.get("metalness")
        roughness = shader_nodes.get("roughness")
        ...
        if "ScanlinesDensity" in Data:
            density = Data["ScanlinesDensity"]
            if isinstance(density, dict) and any(
                    axis in density for axis in ("X", "Y", "Z", "W")
                    ):
                scanlinesDensity = CreateShaderNodeValue(
                    CurMat, scalar_value(density.get("X"), 1.0, "X"), -2200, -50, "ScanlinesDensity.x"
                    )
                scanlinesDensity_y = CreateShaderNodeValue(
                    CurMat, scalar_value(density.get("Y"), 1.0, "Y"), -2200, -100, "ScanlinesDensity.y"
                    )
                scanlinesDensity_z = CreateShaderNodeValue(
                    CurMat, scalar_value(density.get("Z"), 1.0, "Z"), -2200, -150, "ScanlinesDensity.z"
                    )
                scanlinesDensity_w = CreateShaderNodeValue(
                    CurMat, scalar_value(density.get("W"), 1.0, "W"), -2200, -200, "ScanlinesDensity.w"
                    )
            else:
                scanlinesDensity = CreateShaderNodeValue(
                    CurMat, scalar_value(density, 1.0, "ScanlinesDensity"),
                    -2000, 50, "ScanlinesDensity"
                    )
        else:
        ...
        emissiveColor = None
        for key in ("EmissiveColor", "Tint"):
            if key in Data:
                emissiveColor = CreateShaderNodeRGB(CurMat, Data[key], -2000, -850, "EmissiveColor")
                break
        if emissiveColor is None:
            emissiveColor = create_node(
                CurMat.nodes, "ShaderNodeRGB", (-2000, -850), label="EmissiveColor"
            )
            emissiveColor.outputs[0].default_value = (1.0, 1.0, 1.0, 1.0)

        images = {}
        for key, var_name in _IMAGE_PARAM_SPECS:
            if key in Data:
                images[var_name] = self._image_from_rel_path(Data[key])
        parImg = images.get("parImg")
        blinkImg = images.get("blinkImg")
        scanLineImg = images.get("scanLineImg")
        scrollMaskImg = images.get("scrollMaskImg")
        ...
        # viewVector = SC_CameraVectorForward.xyz (parallaxscreen.fx:31). Constant across the
        # surface for a frame, so the layers shift rigidly rather than fanning out toward the
        # screen edges as a per-pixel view direction would.
        cameraForward = create_camera_forward_vector(CurMat, location=(-1850, 250))

        # leftRightDot
        vecDot = create_node(CurMat.nodes, "ShaderNodeVectorMath", (-1500, 200), operation="DOT_PRODUCT")
        CurMat.links.new(cameraForward, vecDot.inputs[0])
        ...
        CurMat.links.new(cameraForward, vecDot2.inputs[0])
        ...
        scroll1 = add_group_node(CurMat, scroll1Group, (-1500, -200), label="scroll1", name="scroll1")
        if "ScrollSpeed1" in Data:
            CurMat.links.new(scrollSpeed1.outputs[0], scroll1.inputs['ScrollSpeed1'])
        if "ScrollStepFactor1" in Data:
            CurMat.links.new(scrollStepFactor1.outputs[0], scroll1.inputs['ScrollStepFactor1'])
        CurMat.links.new(time.outputs[0], scroll1.inputs['Time'])

        scrollUV1Group = _create_scroll_uv_group(1, False)
        scrollUV1 = add_group_node(CurMat, scrollUV1Group, (-1350, -200), label="scrollUV1", name="scrollUV1")
        ...
        CurMat.links.new(vecAdd.outputs[0], scrollUV1X.inputs['newUV'])
        if "ScrollMaskHeight1" in Data:
            CurMat.links.new(scrollMaskHeight1.outputs[0], scrollUV1X.inputs['ScrollMaskHeight1'])
        CurMat.links.new(scroll1.outputs[0], scrollUV1X.inputs['scroll1'])
        if "ScrollMaskStartPoint1" in Data:
            CurMat.links.new(scrollMaskStartPoint1.outputs[0], scrollUV1X.inputs['ScrollMaskStartPoint1'])

        scroll2Group = _create_scroll_group(2)
        scroll2 = add_group_node(CurMat, scroll2Group, (-1500, -300), label="scroll2", name="scroll2")
        if "ScrollSpeed2" in Data:
            CurMat.links.new(scrollSpeed2.outputs[0], scroll2.inputs['ScrollSpeed2'])
        if "ScrollStepFactor2" in Data:
            CurMat.links.new(scrollStepFactor2.outputs[0], scroll2.inputs['ScrollStepFactor2'])
        CurMat.links.new(time.outputs[0], scroll2.inputs['Time'])

        scrollUV2Group = _create_scroll_uv_group(2, False)
        scrollUV2 = add_group_node(CurMat, scrollUV2Group, (-1350, -300), label="scrollUV2", name="scrollUV2")
        CurMat.links.new(vecAdd.outputs[0], scrollUV2.inputs['newUV'])
        if "ScrollMaskHeight2" in Data:
            CurMat.links.new(scrollMaskHeight2.outputs[0], scrollUV2.inputs['ScrollMaskHeight2'])
        CurMat.links.new(scroll2.outputs[0], scrollUV2.inputs['scroll2'])
        if "ScrollMaskStartPoint2" in Data:
            CurMat.links.new(scrollMaskStartPoint2.outputs[0], scrollUV2.inputs['ScrollMaskStartPoint2'])
        ...
        CurMat.links.new(vecAdd.outputs[0], scrollUV2X.inputs['newUV'])
        if "ScrollMaskHeight2" in Data:
            CurMat.links.new(scrollMaskHeight2.outputs[0], scrollUV2X.inputs['ScrollMaskHeight2'])
        CurMat.links.new(scroll2.outputs[0], scrollUV2X.inputs['scroll2'])
        if "ScrollMaskStartPoint2" in Data:
            CurMat.links.new(scrollMaskStartPoint2.outputs[0], scrollUV2X.inputs['ScrollMaskStartPoint2'])
        ...
        # rndOff
        rndOffGroup = bpy.data.node_groups.get('rndOff')
        if rndOffGroup is None:
            rndOffGroup = new_tracked_datablock("node_groups", "rndOff", "ShaderNodeTree")
        ...
        # randomOffset
        randomOffsetGroup = bpy.data.node_groups.get('randomOffset')
        if randomOffsetGroup is None:
            randomOffsetGroup = new_tracked_datablock("node_groups", "randomOffset", "ShaderNodeTree")
        ...
        # newRandomOffset
        newRandomOffsetGroup = bpy.data.node_groups.get('newRandomOffset')
        if newRandomOffsetGroup is None:
            newRandomOffsetGroup = new_tracked_datablock("node_groups", "newRandomOffset", "ShaderNodeTree")
        ...
        # l1/l2/l3
        l1Group = _create_l1_group()
        l1 = add_group_node(CurMat, l1Group, (-950, 300), label="l1")
        CurMat.links.new(vecAdd.outputs[0], l1.inputs[0])

        l2Group = _create_layer_uv_group(2)
        l2 = add_group_node(CurMat, l2Group, (-950, 250), label="l2")
        ...
        l3Group = _create_layer_uv_group(3)
        l3 = add_group_node(CurMat, l3Group, (-950, 200), label="l3")
        ...
        # i2 = SampleIndirectLevel( ParalaxTexture, l2, 2 )
        if "ParalaxTexture" in Data:
            i2Color, _ = create_mip_sampled_texture(
                CurMat, parImg, l2.outputs[0], _LAYER_MIP_LEVELS[2],
                taps=_LAYER_MIP_TAPS, location=(-800, 250), label="i2",
                )
            colorlessTex2 = create_node(CurMat.nodes, "ShaderNodeGroup", (-550, 250), label="colorlessTex")
            colorlessTex2.node_tree = colorlessTexG
            if i2Color is not None:
                CurMat.links.new(i2Color, colorlessTex2.inputs[0])

        # i3 = SampleIndirectLevel( ParalaxTexture, l3, 3 )
        if "ParalaxTexture" in Data:
            i3Color, _ = create_mip_sampled_texture(
                CurMat, parImg, l3.outputs[0], _LAYER_MIP_LEVELS[3],
                taps=_LAYER_MIP_TAPS, location=(-800, 200), label="i3",
                )
            colorlessTex3 = create_node(CurMat.nodes, "ShaderNodeGroup", (-550, 200), label="colorlessTex")
            colorlessTex3.node_tree = colorlessTexG
            if i3Color is not None:
                CurMat.links.new(i3Color, colorlessTex3.inputs[0])
        ...
        bl1Group = bpy.data.node_groups.get('if BlinkingSpeed > 0')
        if bl1Group is None:
            bl1Group = new_tracked_datablock("node_groups", "if BlinkingSpeed > 0", "ShaderNodeTree")
        ...
        # scanlineUV
        scanlineUVGroup = bpy.data.node_groups.get('scanlineUV')
        if scanlineUVGroup is None:
            scanlineUVGroup = new_tracked_datablock("node_groups", "scanlineUV", "ShaderNodeTree")
        ...
        # m2 = (1-(1-i3)*(1-i2));
        m2Group = bpy.data.node_groups.get('parallax_screen_m2')
        if m2Group is None:
            m2Group = new_tracked_datablock("node_groups", "parallax_screen_m2", "ShaderNodeTree")
        ...
        # m3 = (1-(1-m2)*(1-i1));
        m3Group = bpy.data.node_groups.get('parallax_screen_m3')
        if m3Group is None:
            m3Group = new_tracked_datablock("node_groups", "parallax_screen_m3", "ShaderNodeTree")
        ...
        if "ParalaxTexture" in Data:
            parTex2 = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-900, -575), label="ParalaxTexture", image=parImg
                )
        ...
        # finalScrollUV
        finalScrollUVGroup = bpy.data.node_groups.get('finalScrollUV')
        if finalScrollUVGroup is None:
            finalScrollUVGroup = new_tracked_datablock("node_groups", "finalScrollUV", "ShaderNodeTree")
        ...
        # scrollTex
        if "ParalaxTexture" in Data:
            scrollTex = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-1200, -650), label="ParalaxTexture", image=parImg
                )
        ...
        # metalness
        CurMat.links.new(metalness.outputs[0], pBSDF.inputs["Metallic"])
