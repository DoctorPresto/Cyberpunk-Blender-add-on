    ...
    def create(self, Data, Mat):
        ...
        if "Diffuse" in Data:
            diffImg = imageFromRelPath(
                    Data["Diffuse"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
                    )
        ...
        if "RoughMetalBlend" in Data:
            rmbImg = imageFromRelPath(
                    Data["RoughMetalBlend"], self.image_format, DepotPath=self.BasePath, ProjPath=self.ProjPath
                    )
            rmbNode = create_node(
                CurMat.nodes, "ShaderNodeTexImage", (-900, -250), label="RoughnessMetalBlend", image=rmbImg
                )
