...
def source_document_for_filepath(filepath: str) -> dict | None:
    try:
        with DocumentSession() as documents:
            rig_data = RigRepository(documents).load(filepath, required=True)
    except Exception:
        return None
...
def _rig_export_template_from_armature(arm_obj) -> dict:
    arm_data = arm_obj.data
    stored = decode_rig_export_template(str(arm_data.get(RIG_EXPORT_TEMPLATE_KEY, '')))
...
def _compose_qs_matrix(translation: Vector, rotation: Quaternion, scale: Vector) -> Matrix:
    return Matrix.Translation(translation) @ rotation.to_matrix().to_4x4() @ scale_matrix(scale)
...
def _source_pose_data(root: dict, local_array_name: str, model_array_name: str | None):
    ...
    basis = Matrix(RED_LOCAL_TO_BLENDER_BONE_CURRENT)
