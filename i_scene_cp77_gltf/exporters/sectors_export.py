    ...
    try:
        ...
        try:
            ...
            if blender_install_path is not None:
                ...
                messages.append(
                    "If that doesn't help either, run the following command from an administrator command prompt:"
                    )
                messages.append(f"\t\"{blender_python_path}\" -m pip install pyyaml -t \"{blender_module_path}\"")

            messages.append(
                "You can learn more about running Blender scripts under https://tinyurl.com/cp2077blenderpython"
                )
    ...
    for filepath in jsons:
        ...
        for i, e in enumerate(nodes):
            ...
            match type:
                case 'worldInstancedMeshNode':
                    ...
                    instances = [(NodeData, nodeDataIndex) for nodeDataIndex, NodeData in enumerate(t) if
        ...
        if Sector_additions_coll:
            for col in Sector_additions_coll.children:
                                ...
                                if 'Data' in base['worldTransformsBuffer']['sharedDataBuffer'].keys():
                                    ...
                                    for n in nodes:
                                        if n['HandleId'] == str(bufferID - 1):
                                            ref = n
                                    wtbbuffer = \
                                    ref['Data']['worldTransformsBuffer']['sharedDataBuffer']['Data']['buffer']['Data']
                        ...
                        case 'worldInstancedDestructibleMeshNode':
                            ...
                            if (meshname != 0):
                                ...
                                if 'Data' in base['cookedInstanceTransforms']['sharedDataBuffer'].keys():
                                    ...
                                    wtbbuffer = base['cookedInstanceTransforms']['sharedDataBuffer']['Data']['buffer'][
                                        'Data']
                                    bufferID = nodeIndex
                                    ...
                                    for n in nodes:
                                        if n['HandleId'] == str(bufferID - 1):
                                            ref = n
                                    wtbbuffer = \
                                    ref['Data']['cookedInstanceTransforms']['sharedDataBuffer']['Data']['buffer'][
                                        'Data']
                                print('Before = ', len(wtbbuffer['Transforms']))
                                print('inserting at ', idx)
                                wtbbuffer['Transforms'].insert(idx, trans)
                ...
                elif 'nodeIndex' in col.keys() and col['sectorName'] in bpy.data.collections.keys() and len(
                        col.objects
                        ) > 0:
                    match col['nodeType']:
                        case 'worldStaticMeshNode' | 'worldBuildingProxyMeshNode' | 'worldGenericProxyMeshNode' | 'worldTerrainProxyMeshNode':
        ...
        for node in instances_to_copy:
            ...
            if 'cookedInstanceTransforms' in prev_node.keys() or 'worldTransformsBuffer' in prev_node.keys():
                if 'cookedInstanceTransforms' in prev_node.keys() and 'HandleId' in prev_node[
            ...
            if Sector_additions_coll['Inst_bufferID'] > 0:
                new_node['Data']['worldTransformsBuffer']['sharedDataBuffer']['HandleRefId'] = str(
                        Sector_additions_coll['Inst_bufferID']
                        )
                ...
                newbuf = {"BufferId": str(new_Index + 1), "Flags": 4063232,
                          "Type": "WolvenKit.RED4.Archive.Buffer.WorldTransformsBuffer, WolvenKit.RED4.Archive, Version=1.61.0.0, Culture=neutral, PublicKeyToken=null",
                          "Data": {"Transforms": []}}
                new_node['Data']['worldTransformsBuffer']['sharedDataBuffer']['Data'] = {
                    "Data": {"$type": "worldSharedDataBuffer", "buffer": newbuf}}
        ...
        for node in destructibles_to_copy:
            ...
            if 'Dest_bufferID' in Sector_additions_coll.keys() and int(Sector_additions_coll['Dest_bufferID']) > -1:
                ...
                new_node['Data']['cookedInstanceTransforms']['sharedDataBuffer']['HandleRefId'] = str(
                        Sector_additions_coll['Dest_bufferID']
                        )
                ...
                newbuf = {"BufferId": str(new_Index + 1), "Flags": 4063232,
                          "Type": "WolvenKit.RED4.Archive.Buffer.WorldTransformsBuffer, WolvenKit.RED4.Archive, Version=1.61.0.0, Culture=neutral, PublicKeyToken=null",
                          "Data": {"Transforms": []}}
                new_node['Data']['cookedInstanceTransforms']['sharedDataBuffer']['Data'] = {
                    "Data": {"$type": "worldSharedDataBuffer", "buffer": newbuf}}
                print(new_node)
