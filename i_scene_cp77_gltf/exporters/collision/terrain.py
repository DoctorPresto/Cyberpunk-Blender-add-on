...
from .heightfield import PhysXWriter
...
def export_selected_terrain(filePath):
    ...
    if not sector['Data']['RootChunk']['nodes']:
        show_message("No valid terrain collision meshes were selected")
        return {'CANCELLED'}

    atomic_write_json(filePath, sector, indent=4)
