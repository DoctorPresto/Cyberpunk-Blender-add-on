...
def cp77_collision_export(filepath, collision_type):
    ...
    if collision_type == 'VEHICLE':
        ...
        if not collections:
            return {'CANCELLED'}
        return export_colliders_to_phys(collections, filepath)
    elif collision_type == 'TERRAIN':
        return export_selected_terrain(filepath)
    elif collision_type == 'ENTITY':
        show_message('Exporting of Entity Colliders is not yet supported')
        return {'CANCELLED'}
    elif collision_type == 'WORLD':
        show_message('Exporting of collision nodes is not yet supported')
        return {'CANCELLED'}
    show_message(f"Unknown collision export type: {collision_type}")
    return {'CANCELLED'}
