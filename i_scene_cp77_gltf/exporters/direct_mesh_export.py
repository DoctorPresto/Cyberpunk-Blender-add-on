"""Direct Cyberpunk 2077 mesh export to a WolvenKit-compatible GLB.

Writes submesh geometry, skinning, vertex attributes, morph targets and the garment
support layers straight into GLB accessors, bypassing ``bpy.ops.export_scene.gltf``.
The output is the exact inverse of ``direct_mesh_import.decode_mesh_glb``: one mesh per
submesh with a single triangle primitive, per-vertex attributes only, joint nodes under
a single ``Armature`` root, and ``skins[0].extras`` carrying ``boneNames`` /
``boneParentIndexes`` that agree with the emitted node hierarchy.

Blender stores UVs, split normals, tangents and corner colours on the loop domain while
glTF requires them per vertex, so the writer performs its own split-vertex expansion and
remaps deform weights and shape key deltas onto the expanded index set.
"""
...
from .direct_anim_export import GLBBuilder, _encode_glb
from .mesh_validation import VERT_LIMIT, _loop_vertex_indices, _quantize
...
_RED_TO_GLTF_4 = np.linalg.inv(_GLTF_TO_RED_4)
...
_ROTATION_RED_TO_GLTF_T = np.ascontiguousarray(
    np.linalg.inv(_GLTF_TO_RED_4[:3, :3]).T.astype(np.float32)
)
...
def _red_matrix_to_gltf(matrix: np.ndarray) -> np.ndarray:
    return _RED_TO_GLTF_4 @ np.asarray(matrix, dtype=np.float64) @ _GLTF_TO_RED_4
...
def _snapshot_source_bone_names(armature) -> tuple[str, ...]:
    ...
    ``merged_rig_bone_name`` maps authored plug names onto merged slot names and is not
...
def _resolve_target_bone_name(armature, source_name):
    ...
    than attempting to invert merged_rig_bone_name.
    """
    bones = armature.data.bones
    if bones.get(source_name) is not None:
        return source_name
    mapped = merged_rig_bone_name(source_name)
    ...
    for source_name in source_names:
        ...
        if target_name is None:
            raise DirectMeshExportError(
                ...
                f"(also tried {merged_rig_bone_name(source_name)!r})."
            )
...
def assemble_mesh_document(
    ...
    *,
    ...
    document = {
        "asset": {
            "copyright": "",
            "generator": "Cyberpunk 2077 IO Suite direct mesh exporter",
            "version": "2.0",
        },
