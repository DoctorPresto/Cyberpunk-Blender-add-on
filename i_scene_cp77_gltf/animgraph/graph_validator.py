...
from . import graph_index, roundtrip_audit, variable_bindings
...
def _unique_extend(target: List[str], source: Any, *, limit: int) -> None:
    ...
    for item in source:
        ...
        if text in seen:
            continue
        _append_limited(target, text, limit=limit)
        seen.add(text)
        if len(target) >= limit:
            break
...
def _source_alignment_stage(root: Any) -> dict:
    ...
    for key, label in checks:
        ...
        if count:
            _append_limited(blocking, f'{label}: {count}', limit=BLOCKER_LIMIT)
...
def _export_stage(root: Any, payload: Optional[Dict[str, Any]] = None) -> dict:
    try:
        ...
        generated = payload if isinstance(payload, dict) else json_encoder.encode_wolvenkit_json(root)
        audit = json_encoder.export_reversal_audit_for_payload(root, generated)
...
def write_report_to_tree(tree: Any, report: dict) -> None:
    ...
    try:
        ...
        root[SUMMARY_KEY] = str(report.get('artist_summary', '') or report.get('summary', ''))
