...
def _on_anim_variable_updated(self, context):
    try:
        ...
        variable_bindings.on_variable_value_updated(self, context)
    except Exception:
...
def _cname_value(value: Any) -> str:
...
def _handled_payload(value: Any) -> Tuple[str, dict, str, str]:
    ...
    if not isinstance(data, dict):
        return '', {}, '', ''
    t = str(data.get('$type', ''))
    if not t or rtti_schema is None:
        return '', {}, '', ''
    try:
        if rtti_schema.has_class(t) and not rtti_schema.is_graph_node_class(t):
...
def _metadata_type_for(parent_type: str, field: str) -> str:
    ...
    if rtti_schema is not None:
        try:
            for prop in rtti_schema.all_properties(parent_type):
...
def _is_link_type(type_name: str) -> bool:
    ...
    if rtti_schema is not None:
        try:
            return rtti_schema.is_link_type(type_name)
        except Exception:
            return False
    return False
...
def _is_hidden_field(name: str) -> bool:
    ...
    if rtti_schema is not None:
        try:
            return rtti_schema.is_hidden_field(name)
        except Exception:
            pass
    return False
...
def _is_enum_type(type_name: str) -> bool:
    if not type_name or rtti_schema is None:
        return False
    try:
        return rtti_schema.has_enum(type_name)
    except Exception:
        return False


def _resolve_enum_type(type_name: str = '', *, field_name: str = '', parent_type: str = '', json_path: str = '', value: Any = None) -> str:
    if rtti_schema is None:
        return ''
    try:
        return rtti_schema.resolve_enum_type(type_name, field_name=field_name, parent_type=parent_type, json_path=json_path, value=value)
    except Exception:
        return ''
...
def _is_flag_enum(type_name: str) -> bool:
    if rtti_schema is None:
        return False
    try:
        return rtti_schema.is_flag_enum(type_name)
    except Exception:
        return False
...
def _set_vector(target: Any, size: int, values: Iterable[Any]) -> None:
    ...
    vals = list(values)
    for i in range(4):
        target.vector_value[i] = _number(vals[i], 0.0) if i < len(vals) else 0.0


def _read_vec(value: Any, keys: Tuple[str, ...], defaults: Tuple[float, ...]) -> Tuple[float, ...]:
    if not isinstance(value, dict):
        return defaults
    return tuple(_number(value.get(k), defaults[i]) for i, k in enumerate(keys))
...
def _encode_qstransform(target: Any) -> dict:
    return {
        ...
        'Rotation': {
            '$type': 'Quaternion',
            'i': float(target.qs_rotation[0]),
            'j': float(target.qs_rotation[1]),
            'k': float(target.qs_rotation[2]),
            'r': float(target.qs_rotation[3]),
        },
        'Scale': {
            '$type': 'Vector4',
            'X': float(target.qs_scale[0]),
            'Y': float(target.qs_scale[1]),
            'Z': float(target.qs_scale[2]),
            'W': float(target.qs_scale[3]),
        },
        'Translation': {
            '$type': 'Vector4',
            'X': float(target.qs_translation[0]),
            'Y': float(target.qs_translation[1]),
            'Z': float(target.qs_translation[2]),
            'W': float(target.qs_translation[3]),
        },
    }
...
def _decode_struct_fields_into(target: Any, data: dict, struct_type: str, *, value_kind: str = 'STRUCT', handle_id: str = '', ref_id: str = '') -> bool:
    ...
    if rtti_schema is None:
        return False
    try:
        if not rtti_schema.has_class(struct_type):
            return False
    except Exception:
        return False
    ...
    order = {}
    try:
        order = {str(p.get('name')): i for i, p in enumerate(rtti_schema.all_properties(struct_type))}
    except Exception:
        order = {}
...
def _decode_simple_into(target: Any, value: Any, *, red_type_hint: str = '', field_name: str = '', json_path: str = '', parent_type: str = '') -> bool:
    ...
    if enum_type:
        ...
        try:
            if rtti_schema is not None:
                target.enum_raw_value = str(rtti_schema.enum_value_for_name(enum_type, value) if isinstance(value, str) else value)
                target.string_value = rtti_schema.enum_decoded_value_text(enum_type, value)
                choice = rtti_schema.enum_normalize_choice(enum_type, value)
                try:
    ...
    if t == 'animTransformIndex':
        ...
        target.string_value = _cname_value(value.get('name'))
        return True
    ...
    if t == 'animVisualTagCondition':
        ...
        target.string_value = _cname_value(value.get('visualTag'))
        return True
    if t == 'animFloatClamp':
...
def _enum_choice_items(self, context):
    ...
    try:
        return rtti_schema.enum_items(enum_type, include_sentinel=False)
    except Exception:
...
def _on_enum_choice_updated(self, context):
    ...
    try:
        ...
        enum_type = getattr(self, 'enum_type', '') or getattr(self, 'red_type', '')
        if rtti_schema is not None:
            value = rtti_schema.enum_value_for_name(enum_type, choice)
...
class REDengine_AnimCurvePoint(bpy.types.PropertyGroup):
    """One control key in a curveData:Float payload."""
...
class REDengine_AnimArrayField(bpy.types.PropertyGroup):
    """One typed field inside an editable array element."""
...
class REDengine_AnimArrayElement(bpy.types.PropertyGroup):
    """One element in an editable REDengine array property."""
...
class REDengine_AnimNodeProperty(bpy.types.PropertyGroup):
    """One editable REDengine property attached to an imported node."""
...
def _decode_struct_element(element: Any, value: dict, element_type: str) -> bool:
    ...
    order = {}
    if rtti_schema is not None and struct_type:
        try:
            order = {str(p.get('name')): i for i, p in enumerate(rtti_schema.all_properties(struct_type))}
        except Exception:
            order = {}
...
) -> Optional[REDengine_AnimNodeProperty]:
...
def _decode_into_property(item: REDengine_AnimNodeProperty, value: Any, *, red_type_hint: Optional[str] = None) -> None:
...
def _assign_default(item: REDengine_AnimNodeProperty, default: Any) -> None:
...
def encode_simple_value(item: Any) -> Any:
    ...
    if kind in {'ENUM', 'FLAGS_ENUM'}:
        if rtti_schema is not None:
            try:
                return rtti_schema.enum_encode_value(getattr(item, 'enum_type', '') or red_type, getattr(item, 'string_value', ''), storage=getattr(item, 'enum_storage', 'name'), raw_value=getattr(item, 'enum_raw_value', ''))
            except Exception:
                pass
        return str(item.string_value)
    if kind == 'CNAME':
...
def encode_node_property(item: REDengine_AnimNodeProperty) -> Any:
