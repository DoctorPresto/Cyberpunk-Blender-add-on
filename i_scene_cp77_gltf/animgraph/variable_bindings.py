...
def _vector(value: Any, keys: Tuple[str, ...], defaults: Tuple[float, ...]) -> Tuple[float, float, float, float]:
    if not isinstance(value, dict):
        ...
        vals = [_number(value.get(k), defaults[i]) for i, k in enumerate(keys)]
    while len(vals) < 4:
...
def set_variable_from_json(slot: Any, var_type: str, inner: dict, entry: dict, array_name: str = '') -> None:
    ...
    if var_type == 'Bool':
        ...
        slot.default_int = _int(default)
        slot.current_int = _int(value)
    elif var_type == 'Float':
        slot.default_float = _number(default)
        slot.current_float = _number(value)
        slot.has_float_range = _has_float_range_json(inner)
        slot.min_float = _number(inner.get('min'))
        slot.max_float = _number(inner.get('max'))
