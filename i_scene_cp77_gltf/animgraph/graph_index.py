...
def node_handle(node: Any) -> str:
    return str(_get(node, 'red_handle_id', '') or _idprop_get(node, 'red_handle_id', '') or '')


def node_type(node: Any) -> str:
    return str(_get(node, 'red_type', '') or _idprop_get(node, 'red_type', '') or '')
...
def node_is_pseudo(node: Any) -> bool:
    ...
    return bool(
        _truthy(_get(node, 'red_pseudo', False))
...
def socket_exportable(socket: Any) -> bool:
    explicit = _get(socket, 'red_exportable', None)
    return True if explicit is None else bool(explicit)


def socket_semantics(socket: Any) -> str:
    return str(_get(socket, 'red_edge_semantics', '') or 'unknown')


def socket_role(socket: Any) -> str:
    return str(_get(socket, 'red_socket_role', '') or '')


def socket_owner(socket: Any) -> str:
    return str(_get(socket, 'red_owner_handle', '') or '')


def socket_source(socket: Any) -> str:
    return str(_get(socket, 'red_source_handle', '') or '')


def socket_target(socket: Any) -> str:
    return str(_get(socket, 'red_target_handle', '') or '')


def socket_path(socket: Any) -> str:
    return str(_get(socket, 'red_json_path', '') or '')


def socket_link_type(socket: Any) -> str:
    return str(_get(socket, 'red_link_type', '') or '')


def socket_ref_style(socket: Any) -> str:
    return str(_get(socket, 'red_ref_style', '') or '')
    ...
    while queue:
        ...
        for node in getattr(tree, 'nodes', []) or []:
            ...
            for prop_name in ('red_editor_subgraph_a_name', 'red_editor_subgraph_b_name'):
                tname = str(_get(node, prop_name, '') or '')
                if not tname:
                    continue
                try:
...
def _socket_snapshot(socket: Any) -> dict:
    return {
        ...
        'field_name': str(_get(socket, 'red_field_name', '') or ''),
        'json_path': socket_path(socket),
        'link_type': socket_link_type(socket),
        'array_index': int(_get(socket, 'red_array_index', -1) or -1),
        ...
        'pseudo': bool(_get(socket, 'red_pseudo', False)),
    }
    ...
    for tree in iter_trees(root_tree):
        ...
        for node in getattr(tree, 'nodes', []) or []:
            ...
            for prop in getattr(node, 'red_properties', []) or []:
                ...
                kind = str(_get(prop, 'value_kind', '') or 'UNKNOWN')
...
def audit_parser(parser: Any) -> dict:
    ...
    try:
        ...
        for _hid, data in definitions.items():
            ...
            if rtti_schema.has_class(ntype):
                metadata_known += 1
                declared = {p.get('name') for p in rtti_schema.all_properties(ntype)}
...
def load_report_from_tree(root_tree: Any) -> dict:
    if root_tree is None:
        return {}
    raw = _idprop_get(root_tree, 'red_source_alignment_report', '')
    if raw:
        try:
