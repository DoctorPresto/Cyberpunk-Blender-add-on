...
from . import node_builder, rtti_schema
from .constants import ANIMGRAPH_TREE_ID
from .categories import (
    BLEND_TYPES, BONE_TYPES, CLIP_TYPES, CONTAINER_TYPES,
    PHYSICS_TYPES, POSESPACE_TYPES, TERMINATOR_TYPES, node_category,
)
...
class REDENGINE_OT_add_node(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if location is None:
            location = (0.0, 0.0)
        node = node_builder.build_node(tree, self.node_type, location=location)
...
def _all_types():
    return set(rtti_schema.available_node_types())


def _value_types():
    grouped = (BLEND_TYPES | BONE_TYPES | CLIP_TYPES | CONTAINER_TYPES
               | PHYSICS_TYPES | POSESPACE_TYPES | TERMINATOR_TYPES)
    types = {t for t in _all_types() if rtti_schema.output_kind(t) != 'animPoseLink'}
