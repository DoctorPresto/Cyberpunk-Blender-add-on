...
from . import rtti_schema, node_presenters
from .categories import NODE_CATEGORY_COLORS
from .constants import ANIMGRAPH_TREE_ID, CONTAINER_OUTPUT_NAME, LINK_SOCKET
...
def _seed_authored_properties(node, definition: rtti_schema.NodeDefinition) -> None:
...
def build_node(tree, short: str, location=(0.0, 0.0)):
    definition = rtti_schema.node_definition(short)
    hid = new_handle_id()
    ...
    try:
        ...
        node.red_presenter = node_presenters.presenter_id_for(definition.red_type)
    except Exception:
        pass
    ...
    node.color = NODE_CATEGORY_COLORS[definition.category]
    ...
    node_presenters.seed_authored_node(node, definition)
    return node
