...
from . import graph_index, rtti_schema, variable_bindings
try:
    from . import node_presenters
except Exception:
    node_presenters = None
try:
    from .properties import encode_node_property
except Exception:
    encode_node_property = None
...
def _node_name(node: Any) -> str:
    return str(_get(node, 'name', '<node>') or '<node>')


def _node_label(node: Any) -> str:
    return str(_get(node, 'label', '') or _node_name(node))
...
def _append_limited(items: List[str], text: str, *, limit: int) -> None:
    if len(items) < limit:
        items.append(text)
...
def _variable_encode_preview(var: Any) -> Tuple[bool, str]:
    ...
    try:
        ...
        if vt == 'Transform':
            raw = str(_get(var, 'current_json', '') or _get(var, 'current_value', ''))
            if raw:
                try:
                    json.loads(raw)
                    return True, raw
...
def report_for_tree(tree: Any) -> dict:
    ...
    for tree_obj in graph_index.iter_trees(root):
        ...
        for node in getattr(tree_obj, 'nodes', []) or []:
            ...
            presenter = str(_get(node, 'red_presenter', '') or '')
            ...
            if editor and exportable:
                ...
                _append_limited(blockers, text, limit=BLOCKER_LIMIT)
            ...
            if not nhandle:
                _append_limited(blockers, f"runtime node lacks HandleId: {nname} ({ntype or '<no type>'})", limit=BLOCKER_LIMIT)
            if not ntype:
                _append_limited(blockers, f"runtime node lacks red_type: {nname}", limit=BLOCKER_LIMIT)
            elif not ntype.startswith(ANIM_NODE_PREFIX):
                _append_limited(blockers, f"exportable node is not animAnimNode_*: {nhandle or nname} {ntype}", limit=BLOCKER_LIMIT)
            elif not rtti_schema.has_class(ntype):
                _append_limited(blockers, f"runtime node has unknown RTTI class: {nhandle or nname} {ntype}", limit=BLOCKER_LIMIT)
            else:
                counters['runtime_nodes_schema_known'] += 1

            if not presenter:
                _append_limited(blockers, f"runtime node lacks presenter id: {nhandle or nname} {ntype or '<no type>'}", limit=BLOCKER_LIMIT)
            elif not _presenter_known(ntype, presenter):
                _append_limited(warnings, f"runtime node has non-standard presenter id {presenter!r}: {nhandle or nname} {ntype}", limit=WARNING_LIMIT)

            for prop in getattr(node, 'red_properties', []) or []:
                counters['properties_total'] += 1
                key = str(_get(prop, 'key', '') or '')
                path = str(_get(prop, 'json_path', '') or '')
                kind = str(_get(prop, 'value_kind', '') or '')
                ...
                if not kind or kind not in ENCODABLE_KINDS:
                    ...
                    _append_limited(blockers, f"property has unencodable value kind: {label} kind={kind!r}", limit=BLOCKER_LIMIT)
                    continue

                if kind == 'RAW_JSON':
                    counters['raw_properties'] += 1
                    text = f"raw preserved property: {label}"
                    _append_limited(warnings, text, limit=WARNING_LIMIT)
                    details['raw_properties'].append(text)

                if kind in {'ENUM', 'FLAGS_ENUM'}:
                    enum_type = str(_get(prop, 'enum_type', '') or _get(prop, 'red_type', '') or '')
                    if not enum_type or not rtti_schema.has_enum(enum_type):
            ...
            for sock in getattr(node, 'inputs', []) or []:
                ...
                if not owner or not path or not link_type:
                    ...
                    _append_limited(blockers, f"exportable input socket lacks binding: {nhandle or nname}.{getattr(sock, 'name', '<socket>')} owner={owner!r} path={path!r} link={link_type!r}", limit=BLOCKER_LIMIT)
                else:
    ...
    for handle, names in duplicates.items():
        ...
        _append_limited(blockers, f"duplicate HandleId {handle}: {', '.join(map(str, names[:6]))}", limit=BLOCKER_LIMIT)
    ...
    for tree_obj in graph_index.iter_trees(root):
        for link in getattr(tree_obj, 'links', []) or []:
            ...
            if from_sock is None or to_sock is None:
                ...
                _append_limited(blockers, 'broken Blender link object with missing socket', limit=BLOCKER_LIMIT)
                continue
            ...
            if semantics != graph_index.DATAFLOW:
                ...
                if _socket_exportable(from_sock) and _socket_exportable(to_sock):
                    ...
                    _append_limited(blockers, f"non-dataflow link is exportable: {getattr(from_node, 'name', '?')} -> {getattr(to_node, 'name', '?')} semantics={semantics!r}", limit=BLOCKER_LIMIT)
                continue
            ...
            if not source or not target or not path or not link_type:
                ...
                _append_limited(blockers, f"dataflow link lacks identity: {getattr(from_node, 'name', '?')} -> {getattr(to_node, 'name', '?')} source={source!r} target={target!r} path={path!r} type={link_type!r}", limit=BLOCKER_LIMIT)
                continue
            if source not in nodes_by_handle:
                counters['dataflow_links_missing_source_node'] += 1
                _append_limited(blockers, f"dataflow link source handle has no exportable node projection: source={source} target={target}.{path}", limit=BLOCKER_LIMIT)
            if target not in nodes_by_handle:
                counters['dataflow_links_missing_target_node'] += 1
                _append_limited(blockers, f"dataflow link target handle has no exportable node projection: source={source} target={target}.{path}", limit=BLOCKER_LIMIT)
    ...
    for var in root_vars:
        ...
        if ok:
            ...
            _append_limited(blockers, f"variable encode failed: {getattr(var, 'var_type', '?')} {getattr(var, 'name', '<unnamed>')} -> {msg}", limit=BLOCKER_LIMIT)
        ...
        for node in getattr(tree_obj, 'nodes', []) or []:
            try:
                if not variable_bindings.is_variable_node(node):
                    continue
                if not bool(_idprop_get(node, 'red_variable_bound', False)):
    ...
    for text in unbound_nodes[:WARNING_LIMIT]:
        _append_limited(warnings, f"unbound variable reader node: {text}", limit=WARNING_LIMIT)
...
def write_report_to_tree(tree: Any, report: dict) -> None:
    if tree is None:
        return
    root = _root_tree(tree)
    try:
        root[REPORT_KEY] = json.dumps(report, ensure_ascii=False, sort_keys=True)
        root[SUMMARY_KEY] = str(report.get('summary', ''))
        root[READY_KEY] = bool(report.get('ready', False))
    except Exception:
        pass


def load_report_from_tree(tree: Any) -> dict:
    root = _root_tree(tree)
    raw = _idprop_get(root, REPORT_KEY, '')
    if raw:
        try:
...
def run_and_store(tree: Any) -> dict:
    ...
    write_report_to_tree(tree, report)
    return report
