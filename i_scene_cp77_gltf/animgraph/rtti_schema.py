...
from . import categories, metadata_schema, enum_schema
from .constants import ANIM_NODE_PREFIX


LINK_TYPES = metadata_schema.LINK_TYPES
...
def full_name(name_or_short: str) -> str:
    return metadata_schema.full_name(name_or_short)


def short_name(name_or_full: str) -> str:
    return metadata_schema.short_name(name_or_full)


def has_metadata() -> bool:
    return metadata_schema.has_metadata()


def has_class(name_or_short: str) -> bool:
    return metadata_schema.has_class(name_or_short)


def require_class(name_or_short: str) -> dict:
    rec = metadata_schema.get_class(name_or_short)
    if rec is None:
...
def _unwrap_indirect_type(type_name: str) -> str:
    t = str(type_name or '')
...
def is_known_struct_or_payload_type(type_name: str) -> bool:
    t = _unwrap_indirect_type(type_name)
...
def parent_of(name_or_short: str) -> str:
    return metadata_schema.parent_of(name_or_short)


def parent_chain(name_or_short: str, *, include_self: bool = True) -> Tuple[str, ...]:
    return tuple(metadata_schema.parent_chain(name_or_short, include_self=include_self))


def all_properties(name_or_short: str) -> Tuple[dict, ...]:
    return tuple(metadata_schema.all_properties(name_or_short))


def declared_properties(name_or_short: str) -> Tuple[dict, ...]:
    return tuple(metadata_schema.declared_properties(name_or_short))


def property_type_map(name_or_short: str) -> Dict[str, str]:
    return metadata_schema.property_type_map(name_or_short)


def ordered_field_names(name_or_short: str, actual_keys: Iterable[str]) -> List[str]:
    return metadata_schema.ordered_field_names(name_or_short, actual_keys)


def is_hidden_field(field_name: str) -> bool:
    return metadata_schema._is_hidden_field(field_name)


def is_link_type(type_name: str) -> bool:
    return metadata_schema.is_link_type(type_name)


def link_kind_from_type(type_name: str) -> Optional[str]:
    return metadata_schema.link_kind_from_type(type_name)


def output_kind(name_or_short: str) -> Optional[str]:
    return metadata_schema.output_kind(name_or_short)


def is_container(name_or_short: str) -> bool:
    return metadata_schema.is_container(name_or_short)


def is_abstract(name_or_short: str) -> bool:
    return metadata_schema.is_abstract(name_or_short)


def available_node_types(*, concrete_only: bool = True) -> Tuple[str, ...]:
    return tuple(metadata_schema.all_node_short_names(concrete_only=concrete_only))
...
def input_socket_definitions(name_or_short: str, *, seed_array_inputs: bool = True) -> Tuple[InputFieldDef, ...]:
    ...
    for field_name, link_type, is_array in metadata_schema.input_link_fields(name_or_short):
...
def resolve_enum_type(type_name: str = '', *, field_name: str = '', parent_type: str = '', json_path: str = '', value: Any = None) -> str:
    return enum_schema.resolve_enum_type(type_name, field_name=field_name, parent_type=parent_type, json_path=json_path, value=value)


def has_enum(enum_type: str) -> bool:
    return enum_schema.has_enum(enum_type)


def is_flag_enum(enum_type: str) -> bool:
    return enum_schema.is_flags(enum_type)


def enum_items(enum_type: str, *, include_sentinel: bool = False):
    return enum_schema.enum_items(enum_type, include_sentinel=include_sentinel)


def enum_options(enum_type: str, *, include_sentinel: bool = False):
    return enum_schema.options(enum_type, include_sentinel=include_sentinel)


def enum_default(enum_type: str) -> str:
    opts = enum_schema.options(enum_type, include_sentinel=False)
    if not opts:
        return ''
...
def enum_value_for_name(enum_type: str, option_name: str):
    return enum_schema.value_for_name(enum_type, option_name)


def enum_normalize_choice(enum_type: str, value: Any) -> str:
    return enum_schema.normalize_choice(enum_type, value)


def enum_decoded_value_text(enum_type: str, value: Any) -> str:
    return enum_schema.decoded_value_text(enum_type, value)


def enum_encode_value(enum_type: str, value_text: Any, *, storage: str = 'name', raw_value: Any = '') -> Any:
    return enum_schema.encode_value(enum_type, value_text, storage=storage, raw_value=raw_value)


def flags_summary(enum_type: str, value_text: Any, raw_value: Any = '') -> str:
    return enum_schema.flags_summary(enum_type, value_text, raw_value)
...
def property_kind_from_type(type_name: str, *, field_name: str = '', parent_type: str = '', json_path: str = '', value: Any = None) -> Tuple[str, Any, str, str, bool, str]:
    ...
    if t == 'curveData:Float':
        return 'CURVE_FLOAT', metadata_schema.DEFAULT_FLOAT_CURVE, t, '', False, ''
    if t.startswith('handle:') or t.startswith('rRef:'):
        inner = _unwrap_indirect_type(t)
...
def property_definitions(name_or_short: str) -> Tuple[PropertyDef, ...]:
    ...
    for prop in metadata_schema.all_properties(red_type):
        name = str(prop.get('name', ''))
        ptype = str(prop.get('type', ''))
        if not name:
            continue
        hidden = is_hidden_field(name)
        is_link = metadata_schema.is_link_type(ptype)
...
def node_definition(name_or_short: str) -> NodeDefinition:
    ...
    return NodeDefinition(
        ...
        properties=property_definitions(red_type),
        category=categories.node_category(short),
...
def stats() -> dict:
    meta = metadata_schema.stats()
    estats = enum_schema.summary()
    all_defs = available_node_definitions(concrete_only=False) if metadata_schema.has_metadata() else ()
    return {
