...
def find_debugName(obj):
    ...
    if 'debugName' in obj.users_collection[0].keys():
        ...
        if coll_parents.get(obj.users_collection[0].name) != "Scene Collection" and 'debugName' in D.collections[
            coll_parents.get(obj.users_collection[0].name)]:
            debugName = D.collections[coll_parents.get(obj.users_collection[0].name)]['debugName']
        else:
            if coll_parents.get(coll_parents.get(obj.users_collection[0].name.name)) and 'debugName' in D.collections[
                coll_parents.get(coll_parents.get(obj.users_collection[0].name.name))]:
...
def find_nodeType(obj):
    ...
    elif coll_parents.get(obj.users_collection[0].name) != "Scene Collection" and 'nodeType' in D.collections[
        coll_parents.get(obj.users_collection[0].name)]:
...
def new_group(groupname):
    return {"rot": {"pitch": 0, "yaw": 0, "roll": 0}, "pos": {"x": 0, "y": 0, "w": 0, "z": 0}, "childs": [],
            "name": groupname, "isUsingSpawnables": True, "headerOpen": True, "loadRange": 1000, "autoLoad": False,
            "type": "group"}
    ...
    match nodeType:
        ...
        case 'worldEntityNode':
            ...
            if ent.name not in exported:
                rot = {'pitch': float("{:.9g}".format(degrees(ent['ent_rot'][0]))),
                       'roll': float("{:.9g}".format(degrees(ent['ent_rot'][1]))),
                       'yaw': float("{:.9g}".format(degrees(ent['ent_rot'][2])))}
                pos = {'x': float("{:.9g}".format(ent['ent_pos'][0])), 'y': float("{:.9g}".format(ent['ent_pos'][1])),
                       'z': float("{:.9g}".format(ent['ent_pos'][2])), 'w': float("{:.9g}".format(0))}
        ...
        case "worldRotatingMeshNode":
            OS_obj = new_object(
                "mesh/rotatingMesh", obj.name, set_pos(obj), set_rot(obj), obj.users_collection[0]['mesh']
                )
        ...
        case _:
            if obj.users_collection[0].name not in exported and 'mesh' in obj.users_collection[0].keys():
                ...
                if GENERATE_COLLISIONS == 'ALL' or (GENERATE_COLLISIONS == 'STRUCT' and (
                        'wall' in name and 'decal' not in name and 'door' not in name or 'floor' in name or 'ceil' in name)):
