...
def CP77CollisionTriangleMeshJSONimport(jsonpath):
    ...
    addon_package = next(
        (
            ...
            if name == "i_scene_cp77_gltf" or name.endswith(".i_scene_cp77_gltf")
        ),
        None,
    )
    if addon_package is None:
        raise RuntimeError("Cyberpunk 2077 IO Suite is not enabled")
    cp77_addon_prefs = bpy.context.preferences.addons[addon_package].preferences
    ...
    mesh_obj[
        'physics_material'] = 'plastic.physmat'  # NOT in the json from the geom cache, need to pull it from the sector
...
for key, match in Matches.items():
    ...
    meshpath = os.path.join(
        path, match['meshname'][:-1 * len(os.path.splitext(match['meshname'])[1])] + '.glb'
        ).replace('\\', os.sep)
    print(meshpath)
    ...
    if imported:
        ...
        if y == 0:
            currentcolmesh = CP77CollisionTriangleMeshJSONimport_by_hashes(
                sectorHashStr=sectorHashStr, entryHashStr=entryHashStr, project_raw_dir=path
                )
            o = currentcolmesh
        else:
        ...
        if abs(mdim[0] - coldim[0]) < 0.5 and abs(mdim[1] - coldim[1]) < 0.5 and abs(
                mdim[2] - coldim[2]
                ) < 0.5 and points_within_tol(colcenter, meshcenter, tolerance=0.65):
...
for key in list(Matches.keys()):
    if Matches[key]['count'] == 0:
        ...
        match_text_data.write(
            Matches[key]['colname'] + ',' + Matches[key]['meshname'] + ',' + str(Matches[key]['count']) + '\n'
            )
