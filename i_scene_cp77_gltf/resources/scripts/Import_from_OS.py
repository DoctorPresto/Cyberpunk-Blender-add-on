...
def get_position(obj):
    pos = Vector(
            (obj['spawnable']['position']['x'], obj['spawnable']['position']['y'], obj['spawnable']['position']['z'])
            )
    rot = Euler(
            (radians(obj['spawnable']['rotation']['pitch']),
...
def process_object(obj, parent_coll):
    ...
    if spawndata[-5:] == '.mesh':
        meshpath = os.path.join(
            project_path, 'source', 'raw', spawndata[:-1 * len(os.path.splitext(spawndata)[1])] + '.glb'
            ).replace('\\', os.sep)
            ...
            if ShapeType == 'Box':
                ...
                if ShapeType == 'Box':
                    bpy.ops.mesh.primitive_cube_add(
                        size=1, scale=(float(extents['x']) * 2, float(extents['y']) * 2, float(extents['z']) * 2),
