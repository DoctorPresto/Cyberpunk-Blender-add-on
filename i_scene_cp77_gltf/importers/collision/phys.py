...
from ...assetio.values import cname_value, vector3
from ...blender.collections import newly_linked_collection_object
from ...physics import PhysicsRepository, PhysicsResource
...
def _vector3(value, default=(0.0, 0.0, 0.0)):
    return Vector(vector3(value, default))
...
def _actor_type(body, params):
    simulation_type = cname_value(params.get('simulationType')) or cname_value(body.get('simulationType'))
...
def _import_phys_resource(
    ...
    if not isinstance(resource, PhysicsResource):
        raise TypeError('resource must be a PhysicsResource')
    bodies = resource.bodies
    handle_lookup = resource.handles
    ...
    for body_index, body_ref in enumerate(bodies):
        ...
        body_name = cname_value(body.get('name'), f'PhysicsBody_{body_index}')
        ...
        for shape_index, shape_ref in enumerate(collision_shapes):
            ...
            if actor_obj is None:
                actor_obj = newly_linked_collection_object(target_collection, existing_objects, body_name)
                if actor_obj is None:
    ...
    non_verbose = bool(preferences and preferences.non_verbose)
    if not non_verbose:
        elapsed = time.time() - start_time
        print(f'Registered {actor_count} physics body actors with {shape_count} shapes in {elapsed} seconds')


def import_phys(filepath, rig=None, chassis_z=None):
    with DocumentSession() as documents:
        resource = PhysicsRepository(documents).load(filepath, required=True)
    return _import_phys_resource(resource, rig=rig, chassis_z=chassis_z)


def import_phys_into_collection(
    resource,
    target_collection,
    actor_matrix=None,
    context=None,
