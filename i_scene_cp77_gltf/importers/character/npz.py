...
from ...paths import get_char_dir
...
def _build_mesh_object(context, data, prefix="", obj_name=None, link_to_collection=None):
    ...
    mesh = track_created_datablock("meshes", bpy.data.meshes.new(obj_name))
    obj = track_created_datablock("objects", bpy.data.objects.new(obj_name, mesh))
    ...
    for m_name in mat_names:
        mat = bpy.data.materials.get(m_name) or track_created_datablock("materials", bpy.data.materials.new(name=m_name))
...
def load_mesh_npz(context, filepath, link_to_collection=None):
    ...
    if version >= 2 and "submesh_count" in data:
        ...
        coll = track_created_datablock("collections", bpy.data.collections.new(coll_name))
