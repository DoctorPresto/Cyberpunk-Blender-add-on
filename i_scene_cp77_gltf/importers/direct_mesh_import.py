...
def _node_hierarchy(document: dict):
    ...
    for node_index in topology:
        ...
        global_matrices[node_index] = (
            local[node_index]
            if parent_index < 0
            else global_matrices[parent_index] @ local[node_index]
        )
...
def _red_model_matrix(gltf_matrix):
    return _GLTF_TO_RED_4 @ gltf_matrix @ _RED_TO_GLTF_4
...
def _build_mesh_geometry(submesh: Submesh):
    mesh = bpy.data.meshes.new(submesh.name)
...
def _new_submesh_object(submesh: Submesh):
    ...
    return bpy.data.objects.new(submesh.name, mesh), loop_vertices
...
def _bind_failure_detail(armature, skin, expected_matrices):
    ...
    if short:
        report.append(
            ...
            f"this tolerance; lengthen them (see read_rig._precision_safe_bone_length)."
        )
    if non_rigid:
        report.append(
...
def _set_created_armature_bind_pose(armature, skin, expected_matrices):
    ...
    from .read_rig import set_armature_model_space_matrices
...
def _new_import_collection(filepath: str):
    collection = bpy.data.collections.new(_collection_name_from_filepath(filepath))
